...
@php
    ...
    $active = $altActive ? (request()->is(\Illuminate\Support\Str::beforeLast($altActive, '*')) || request()->is(\Illuminate\Support\Str::beforeLast($altActive, '*') .'/*')) : $active;
...
{{--            <x-dynamic-component component="{{ $icon }}" class="mr-3 h-6 w-6 text-gray-200"/>--}}
        ...
        @if ($sub && $active && !$icon)
            <svg class="mr-5 h-4 w-4 text-white-200" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">

    ...
    <x-move-dropdown orientation="left" wire:key="table-filter.checkbox">
        <x-slot name="trigger">
            <button class="flex items-center cursor-pointer active:border relative mt-2" type="button">
                <span class="form-checkbox border rounded-md @if ($table->has_selected) bg-primary-600 @endif h-5 w-5 text-primary-600 cursor-pointer">
        ...
        <div class="px-3">
            ...
            <label class="inline-flex items-center my-3">
                ...
                <span class="ml-2 text-gray-700 text-sm">Alle overeenkomende selecteren ({{ $table->total }})</span>
            </label>
        </div>
    </x-move-dropdown>

    <div class="flex items-center flex-wrap items-stretch">
        @if ($table->has_selected && count($table->actions() ?? []))
            <select class="form-control form-select mr-2 py-1" wire:model="action">
                ...
                <optgroup label="{{ $table->resource()->label() }}">
                @foreach($table->actions() as $i => $action)
            ...
            <x-move-dialog-modal wire:model="showingAction">
                <x-slot name="button">
                    <button class="active:border @if ($table->action !== '-') bg-primary-600 hover:bg-primary-500 cursor-pointer @else bg-gray-200 disabled cursor-default @endif py-1 px-4 rounded-md mr-2"
                            x-on:click="show = true"
                            wire:loading.attr="disabled"
                            @if ($table->action === '-') disabled="disabled" @endif
                    >
        ...
        <x-move-dropdown orientation="right" wire:key="table-filter.filters">
            <x-slot name="trigger">
                <button type="button" class="flex items-center cursor-pointer active:border relative items-center cursor-pointer @if ($table->has_filters) bg-primary-600  hover:bg-primary-500 @else bg-gray-100 @endif py-1 pr-2 pl-1 rounded-md mr-2">
                    <svg class="@if ($table->has_filters) text-white @else text-gray-600 @endif  h-6 w-6 ml-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                    </svg>
                    @if ($table->has_filters) <span class="text-white px-2">{{ $table->has_filters }}</span> @endif
                    <svg class="@if ($table->has_filters) text-white @else text-gray-600 @endif h-4 w-4 ml-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
            </x-slot>

...
>
    ...
    <div class="relative w-full text-gray-400 focus-within:text-gray-600">
        ...
        <input autocomplete="off" x-on:click="open = true" wire:model="search" id="search_field" class="border-none block w-full h-full pl-8 pr-3 py-2 rounded-md text-gray-900 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 sm:text-sm" placeholder="{{ __('Press / to search') }}" type="search">
    </div>
                    ...
                    <ul>
                        @foreach ($result['result'] as $model)
                            @if (!is_object($model))
                                @continue
                            @endif
                            <li class="w-full focus:bg-primary-100 hover:bg-primary-50">
